...
Common recoverable shape mistakes are normalized before validation: a command accidentally placed at the start of `args` is moved into `executable`, numeric strings become integers, a file passed as `cwd` becomes its parent directory, and fields copied from another editing tool are removed when the intended edit is otherwise complete. The common `patch_file` alias maps to `apply_patch`. An identical whole-file write returns `unchanged` without creating another transaction or approval, and repeated calls that produce the exact same outcome receive a stop-and-change-approach warning.
