...
test('a stale whole-file write is forced through a fresh read and repaired safely', async (t) => {
  const app = await fixture(t, (body, round) => {
    ...
    if (round === 2) {
      ...
      assert.match(body.messages.at(-1).content, /path "note\.txt"/);
      assert.ok(body.messages.some((message) => message.role === 'user' && message.content === 'Replace the complete note.txt file.'));
      assert.ok(body.messages.some((message) => message.role === 'tool' && message.tool_call_id === 'stale-write'));
...
test('Continue keeps the earlier actionable request in tool recovery context', async (t) => {
  const app = await fixture(t, (body, round) => {
    ...
    assert.equal(body.tool_choice, 'required');
    assert.deepEqual(body.tools.map((tool) => tool.function.name), ['apply_patch']);
    assert.match(body.messages.at(-1).content, /active objective "Fix note\.txt by replacing its text\."/);
    assert.ok(body.messages.some((message) => message.role === 'user' && message.content === 'Fix note.txt by replacing its text.'));
    assert.ok(body.messages.some((message) => message.role === 'user' && message.content === 'Continue.'));
    assert.doesNotMatch(body.messages.at(-1).content, /Emit the required tool call now/);
...
test('long active tool runs checkpoint oversized tool arguments without losing the user objective', async (t) => {
  const oversized = 'x'.repeat(30_000);
  const app = await fixture(t, (body, round) => {
    if (round === 1) {
      return { tool_calls: [call('oversized-invalid-read', 'read_file', { path: 'note.txt', unexpected: oversized })] };
    }
    const checkpoint = body.messages.find((message) => message.role === 'system' && String(message.content).startsWith('Active-turn checkpoint generated by Snowyy'));
    assert.ok(checkpoint, 'the next provider request should contain an active-turn checkpoint');
    assert.match(checkpoint.content, /Active objective:\nInspect note\.txt and explain what happened\./);
    assert.match(checkpoint.content, /read_file failed/);
    assert.ok(body.messages.some((message) => message.role === 'user' && message.content === 'Inspect note.txt and explain what happened.'));
    assert.equal(body.messages.some((message) => message.role === 'tool'), false, 'the oversized completed exchange should be archived');
    return { content: 'The read failed because the tool arguments did not match its schema.' };
  });
  const stream = await app.chat('Inspect note.txt and explain what happened.', { maxContextTokens: 6_000 });
  const checkpointEvent = events(stream, 'compacted').find((event) => event.scope === 'active_turn');
  assert.ok(checkpointEvent);
  assert.ok(checkpointEvent.estimatedTokensBefore > checkpointEvent.estimatedTokensAfter);
  assert.equal(events(stream, 'done').length, 1);
  const sessionId = events(stream, 'session')[0].id;
  const session = (await (await fetch(`${app.url}/api/sessions/${sessionId}`)).json()).session;
  assert.equal(session.timeline.some((entry) => entry.type === 'compaction' && entry.scope === 'active_turn'), true);
});
