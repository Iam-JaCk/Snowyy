...
export const editingToolDefinitions = [
  {
    ...
    function: {
      name: 'apply_patch',
      description: 'Replace one unique exact block copied from read_file. Best for targeted edits. Use only old_text and new_text; do not add line-range fields. Replacement is literal, not a unified diff. Use more surrounding context if the block occurs more than once.',
  ...
  },
  {
    ...
    function: {
      name: 'write_file',
      description: 'Create a new file or deliberately replace an entire existing file. The empty hash means the file must not exist; it never means overwrite. Existing files require the exact latest SHA-256 from a complete read_file result. Never invent a hash or use a sha256: prefix. Requires approval.',
      parameters: { type: 'object', properties: {
        path: { type: 'string', description: 'Workspace-relative file path.' },
        content: { type: 'string', description: 'Complete new UTF-8 file content.' },
        expected_sha256: { type: 'string', description: 'Use \"\" only when the path does not exist. To replace an existing file, use the exact 64-character hash from the latest complete read_file call.' }
  ...
  },
  {
    ...
    function: {
      name: 'replace_lines',
      description: 'Replace an inclusive line range using start_line, end_line and new_text from the latest read_file result. Do not add old_text. A trailing newline in new_text is accepted; empty new_text deletes the range.',
      parameters: { type: 'object', properties: {
  ...
  },
  {
    ...
    function: {
      ...
      description: 'Roll back a recent Snowyy edit transaction if files have not changed since it ran. Repeating an already completed rollback returns its prior result without changing files again. Requires approval.',
...
export function createEditingRegistry(sandbox) {
  ...
  const completedRollbacks = new Map();
  ...
  function checkExpected(state, expected) {
    ...
    if (state.exists && expected === '') throw toolError('EXPECTED_HASH_REQUIRED', `Existing file requires its current read_file SHA-256: ${state.path}.`, 'Call read_file and use its exact sha256. The empty string is only for a path that does not exist.');
    if (state.exists && !/^[a-f0-9]{64}$/i.test(expected)) throw toolError('INVALID_FILE_HASH', `expected_sha256 is not a valid SHA-256 for ${state.path}.`, 'Call read_file and copy its exact 64-character sha256 without a prefix.');
  ...
  async function commit(name, args, { approvedPreview, signal } = {}) {
    ...
    if (files.every((file) => file.exists && file.after === file.content)) {
      return {
        unchanged: true,
        message: 'No file content changed because the requested content already matches the workspace. Do not repeat the same edit.',
        files: files.map((file) => ({
          path: sandbox.relative(file.absolute),
          created: false,
  ...
  async function rollback({ transaction_id: transactionId }) {
    ...
    if (!files) {
      const previous = completedRollbacks.get(transactionId);
      if (previous) return { rolled_back: transactionId, files: [...previous], already_rolled_back: true };
      throw toolError('ROLLBACK_NOT_FOUND', 'Rollback transaction was not found or has expired.', 'Use a transaction_id from a successful edit in the current app session. Do not invent or alter it.');
    }
    for (const file of files) {
      const current = await fileState(file.path, true);
    ...
    const restoredPaths = files.map((file) => file.path);
    completedRollbacks.set(transactionId, restoredPaths);
    while (completedRollbacks.size > 30) completedRollbacks.delete(completedRollbacks.keys().next().value);
    return { rolled_back: transactionId, files: restoredPaths, already_rolled_back: false };
  }
