...
function splitLiteralCommand(command) {
  ...
  for (let index = 0; index < input.length; index += 1) {
    ...
    if (quote) {
      ...
      else if (character === '\\' && input[index + 1] === quote) token += input[++index];
...
export async function normalizeToolArguments(state, name, originalArgs) {
  ...
  const adjustments = [];
  const fieldAliases = {
    file_path: 'path', filePath: 'path', oldText: 'old_text', old_string: 'old_text',
    newText: 'new_text', new_string: 'new_text',
    expectedHash: 'expected_sha256', expectedSha256: 'expected_sha256', startLine: 'start_line',
    endLine: 'end_line', lineNumber: 'line', workingDirectory: 'cwd', workdir: 'cwd', timeoutMs: 'timeout_ms',
    fileQuery: 'file_query', caseSensitive: 'case_sensitive', maxChars: 'max_chars',
    goalId: 'goal_id', workflowId: 'workflow_id', stepId: 'step_id', stepStatus: 'step_status',
    transactionId: 'transaction_id', changeId: 'transaction_id', change_id: 'transaction_id'
  };
  for (const [alias, canonical] of Object.entries(fieldAliases)) {
    if (!Object.hasOwn(args, alias)) continue;
    if (!Object.hasOwn(args, canonical)) args[canonical] = args[alias];
    delete args[alias];
    adjustments.push(`Renamed ${alias} to ${canonical}.`);
  }
  if (name === 'run_command') {
    if ((!args.executable || typeof args.executable !== 'string') && typeof args.command === 'string') {
      const tokens = splitLiteralCommand(args.command);
      if (tokens) {
        [args.executable, ...args.args] = tokens;
        delete args.command;
        adjustments.push('Split the command string into executable and literal args without using a shell.');
      }
    }
    ...
    if (args.executable === 'tsc') {
      args.executable = 'npx';
      args.args = ['tsc', ...(Array.isArray(args.args) ? args.args : [])];
      adjustments.push('Invoked tsc through the allowed npx executable.');
    }
  }
