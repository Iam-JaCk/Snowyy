...
export const toolDefinitions = [
  ...
  define('list_workflows', 'List persistent workflows, their ordered steps, and progress for the current session. Read-only.', {}),
  define('create_workflow', 'Create a persistent ordered workflow for a task with multiple meaningful stages. Do this once, then update each step as work progresses.', {
    title: { type: 'string', minLength: 1, maxLength: 80, description: 'Short workflow objective.' },
    steps: { type: 'array', minItems: 2, maxItems: 12, items: { type: 'string', minLength: 1, maxLength: 96 }, description: 'Two to twelve concrete ordered step titles.' }
  }, ['title', 'steps']),
  define('update_workflow', 'Rename a workflow, complete it, or update one step. Completing or skipping the active step automatically activates the next pending step.', {
    workflow_id: { type: 'string', minLength: 1 },
    title: { type: 'string', minLength: 1, maxLength: 80 },
    status: { type: 'string', enum: ['active', 'complete'] },
    step_id: { type: 'string', minLength: 1 },
    step_status: { type: 'string', enum: ['pending', 'active', 'complete', 'skipped'] }
  }, ['workflow_id']),
  define('delete_workflow', 'Remove a session workflow that is no longer useful.', {
    workflow_id: { type: 'string', minLength: 1 }
  }, ['workflow_id']),
  define('set_mode', 'Switch this session between plan mode and agent mode. Use plan to inspect and design without mutations; use agent when the user wants the planned work implemented.', {
    mode: { type: 'string', enum: ['plan', 'agent'] },
    reason: { type: 'string', maxLength: 200, description: 'Brief reason for the mode change.' }
  }, ['mode']),
  define('run_command', 'Run a command with a literal argument array, subject to session approval settings. Put the program in executable and only its arguments in args. Example: {"executable":"npx","args":["tsc","--noEmit"]}. No shell pipelines or redirection. Inspect ok, exit_code, stdout and stderr; nonzero exits and timeouts are failures.', {
    executable: { type: 'string', enum: ALLOWED_EXECUTABLES, description: 'Program to run: node, npm, npx, git, or rg. This field is required.' },
...
export function createToolRegistry(workspaceRoot, { web = createWebTools() } = {}) {
  ...
  const registry = new Map([
    ...
    ['list_workflows', { approval: false, execute: (_args, context) => context.workflows.list() }],
    ['create_workflow', { approval: false, execute: (args, context) => context.workflows.create(args) }],
    ['update_workflow', { approval: false, execute: (args, context) => {
      if (args.title === undefined && args.status === undefined && args.step_id === undefined) {
        throw toolError('INVALID_ARGUMENTS', 'update_workflow requires title, status, or a step update.', 'Provide the workflow field that should change.');
      }
      if ((args.step_id === undefined) !== (args.step_status === undefined)) {
        throw toolError('INVALID_ARGUMENTS', 'step_id and step_status must be provided together.', 'Provide both fields for a workflow step update.');
      }
      return context.workflows.update(args.workflow_id, args);
    } }],
    ['delete_workflow', { approval: false, execute: (args, context) => context.workflows.remove(args.workflow_id) }],
    ['set_mode', { approval: false, execute: (args, context) => context.session.setMode(args.mode, args.reason) }],
...
export function summarizeToolCall(name, args = {}) {
  ...
  if (name === 'create_workflow') return { title: 'Create workflow', detail: args.title || '', risk: 'local' };
  if (name === 'update_workflow') return { title: 'Update workflow', detail: args.workflow_id || '', risk: 'local' };
  if (name === 'delete_workflow') return { title: 'Delete workflow', detail: args.workflow_id || '', risk: 'local' };
  if (name === 'list_workflows') return { title: 'List workflows', detail: '', risk: 'read' };
  if (name === 'set_mode') return { title: `Switch to ${args.mode || 'session'} mode`, detail: args.reason || '', risk: 'local' };
